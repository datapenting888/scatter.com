...
async function showCenterNotif(text, duration = 3000) {
  ...
  // --- Deteksi Tema untuk Gambar Notifikasi ---
  const isJujutsuTheme = document.body.classList.contains('bg-jujutsu');
  const isAvatarTheme = document.body.classList.contains('bg-avatar');
  const isNarutoTheme = document.body.classList.contains('bg-naruto');
  const isOnePieceTheme = document.body.classList.contains('bg-onepiece');
  const isKurokoTheme = document.body.classList.contains('bg-kuroko');
  
  let localSrc;
  let onlineSrc;
  
  if (isJujutsuTheme) {
    // === KHUSUS UNTUK TEMA JUJUTSU KAISEN ===
    localSrc = './Jujutsukaisen.jpg';
    onlineSrc = 'https://i.ibb.co/3k5b8Xv/Jujutsukaisen.jpg'; 
  } else if (isAvatarTheme) {
    // === KHUSUS UNTUK TEMA AVATAR AIRBENDER ===
    localSrc = './Avataraang.jpg';
    onlineSrc = 'https://i.ibb.co/5G6vQr7/Avataraang.jpg'; 
  } else if (isNarutoTheme) {
    // === KHUSUS UNTUK TEMA NARUTO ===
    localSrc = './NarutoBaryon.jpg';
    onlineSrc = 'https://i.ibb.co/.../NarutoBaryon.jpg'; 
  } else if (isOnePieceTheme) {
    // === KHUSUS UNTUK TEMA ONE PIECE ===
    localSrc = './OnePiece.jpg';
    onlineSrc = 'https://i.ibb.co/.../OnePiece.jpg'; 
  } else if (isKurokoTheme) {
    // === KHUSUS UNTUK TEMA KUROKO NO BASKET ===
    localSrc = './Kuroko.jpg';
    onlineSrc = 'https://i.ibb.co/.../Kuroko.jpg'; 
  } else {
    // === KHUSUS UNTUK TEMA LAPAK99 (DEFAULT) ===
    localSrc = './LogoLapak.png';
    onlineSrc = 'https://i.ibb.co/h7h3Hdf/Dasha-taran.jpg';
  }

  notif.innerHTML = `
    <img class="notif-img-el" src="${localSrc}" 
         onerror="this.src='${onlineSrc}'; this.onerror=null;"
         style="width:180px; height:180px; border-radius:25px; border:3px solid #cc88ff; 
                box-shadow:0 0 30px rgba(204,0,255,0.6); margin-bottom:15px; 
...
async function playLightningAnimation(customMsg = '') {
  return new Promise((resolve) => {
    ...
    // =========================================================
    // --- PERUBAHAN: Palet Warna Petir Khusus Jujutsu Kaisen ---
    const isJujutsuTheme = document.body.classList.contains('bg-jujutsu');
    
    let palette;
    if (isJujutsuTheme) {
      // Warna Jujutsu Kaisen (Ungu Neon, Pink, Putih)
      palette = {
        step1: '#cc00ff', // Dari Tombol ke UserID (Ungu Neon)
        step2: '#ff00aa', // Dari UserID ke TxID (Pink Neon)
        step3: '#ffffff', // Petir Utama ke Moon (Putih)
        step3Branch: '#8800cc', // Cabang petir ke Moon (Ungu Tua)
        flash: 'rgba(204, 0, 255, 0.25)' // Screen flash (Ungu transparan)
      };
    } else {
      // Warna Default (Biru Neon, Emas, Putih)
      palette = {
        step1: '#00d4ff',
        step2: '#ffd700',
        step3: '#ffffff',
    ...
    function createImpactSparks(x, y, count = 8, color = '#ffffff') {
    ...
    function screenFlash(color = 'rgba(255, 255, 255, 0.15)', duration = 120) {
    ...
    screenFlash(palette.flash, 100);
    drawBolt(btnCenter.x, btnCenter.y, userCenter.x, userCenter.y, palette.step1, 500);
    createImpactSparks(userCenter.x, userCenter.y, 6, palette.step1);
    userIdEl.style.boxShadow = `0 0 20px ${palette.step1}, inset 0 0 10px ${palette.step1}`;

    setTimeout(() => {
      // Step 2: Lightning from UserID to TicketCode
      drawBolt(userCenter.x, userCenter.y, ticketCenter.x, ticketCenter.y, palette.step2, 500);
      createImpactSparks(ticketCenter.x, ticketCenter.y, 6, palette.step2);
      ticketEl.style.boxShadow = `0 0 20px ${palette.step2}, inset 0 0 10px ${palette.step2}`;
      ...
      setTimeout(() => {
        ...
        screenFlash(palette.flash, 150);
        drawBolt(ticketCenter.x, ticketCenter.y, moonCenter.x, moonCenter.y, palette.step3, 600);
        drawBolt(ticketCenter.x + 10, ticketCenter.y - 5, moonCenter.x - 10, moonCenter.y + 5, palette.step3Branch, 550);
        createImpactSparks(moonCenter.x, moonCenter.y, 12, palette.step3);
...
// ============================================================
// === LISTENER UNTUK UPDATE TEMA DARI BACKGROUND ===
// ============================================================
window.addEventListener('message', (event) => {
  const data = event?.data || {};
  
  // Tangani pesan pergantian font dari bridge
  if (data.type === 'SCATTER_UPDATE_THEME') {
    const theme = data.theme;
    if (theme === 'jujutsu') {
      setJujutsuFont(true);
      setAvatarFont(false);
      setNarutoFont(false);
      setOnePieceFont(false);
      setKurokoFont(false);
    } else if (theme === 'avatar') {
      setJujutsuFont(false);
      setAvatarFont(true);
      setNarutoFont(false);
      setOnePieceFont(false);
      setKurokoFont(false);
      ...
      setAvatarFont(false);
      setNarutoFont(false);
      setOnePieceFont(true);
      setKurokoFont(false);
    } else if (theme === 'kuroko') {
      setJujutsuFont(false);
      setAvatarFont(false);
      setNarutoFont(false);
      setOnePieceFont(false);
