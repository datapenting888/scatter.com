...
function ensureBonussmbAlarm() {
    try {
        chrome.alarms.create(BONUSSMB_AUTO_ALARM, { periodInMinutes: 3 });
    } catch {
    }
}
...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
    ...
    // === TAMBAHAN: SYNC THEME FONT DARI POPUP ===
    if (msg.action === 'syncThemeFont') {
        // Kirim instruksi ke semua tab yang membuka index.html (scatter page)
        const theme = msg.theme;
        chrome.tabs.query({ url: chrome.runtime.getURL('index.html') }, (tabs) => {
            for (const tab of tabs) {
                chrome.tabs.sendMessage(tab.id, { 
                    action: 'updateThemeFont', 
                    theme: theme 
                }).catch(() => {
                    // Tab mungkin belum inject bridge-scatter.js, abaikan error
                });
            }
        });
